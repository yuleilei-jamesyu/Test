...
use strict;
#======================================================================
# Description: manipulate and sent out the scorecard team yearly report.
...
my %env_opts = (
                'development' =>
                    '../conf/team_yearly_report_development.conf',
                'test'        =>
                    '../conf/team_yearly_report_test.conf',
                'production'  =>
                    '../conf/team_yearly_report_production.conf',
               );
...
foreach my $quarter ( @{$quarters->{'sorter'}} ) {
    ...
    while ( my $select_scores_rh = $select_scores_sth->fetchrow_hashref ) {
        ...
        $scorecard_report{$coach_name}{$coach}{$author_name}{$quarter}
{$article_id}{$unique_id}
        = \%score;

        push @{$scorecard_report{$coach_name}{$coach}{$author_name}{$quarter}
{$article_id}{'sorter'}},
        $unique_id;

        # sorting author names
        if ( !grep { $_ eq $author_name } @{$scorecard_report{$coach_name}
{$coach}{'sorter'}}  ) {
        ...
        # sorting quarters
        if( !grep { $_ eq $quarter } @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}}) {
...
{'sorter'}}, $quarter;
        ...
        # sorting article ids
        if ( !grep { $_ eq $article_id } @{$scorecard_report{$coach_name}
{$coach}{$author_name}{$quarter}{'sorter'}} ) {
            push @{$scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{'sorter'}}, $article_id;
        }
...
foreach my $coach_name ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$coach_name}} ) {
        ...
        foreach my $author_name
            ...
            foreach my $quarter ( @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}} ) {
...
{$quarter}{$article_id}{'new articles'}
                    = "0";
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'edited articles'}
                    = "0";

                    foreach my $unique_id ( @{$scorecard_report{$coach_name}
{$coach}{$author_name}{$quarter}{$article_id}{'sorter'}} ) {

                        my $created_ts = $scorecard_report{$coach_name}{$coach}
{$author_name}{$quarter}{$article_id}{$unique_id}{'created'};
...
{$author_name}{$quarter}{$article_id}{'new articles'} = "1";
                            }
                            elsif ($edited_flag > 0) {
                                $edited_article_scored ++;
                                $scorecard_report{$coach_name}{$coach}
{$author_name}{$quarter}{$article_id}{'edited articles'} = "1";
                            }
                        }
                        else {
                            $created_flag
...
{$author_name}{$quarter}{$article_id}{'new articles'} = "1";
                    ...
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'new article scored'}
                    = $new_article_scored;
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'edited article scored'}
                    = $edited_article_scored;

                    $total_times_scored
                    = $new_article_scored + $edited_article_scored;
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'total times scored'}
                    = $total_times_scored;
...
{$author_name}{$quarter}{$article_id}{$unique_id}{'created'};
                            my $score
                            = $scorecard_report{$coach_name}{$coach}
{$author_name}{$quarter}{$article_id}{$unique_id}{'score'};

                            $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'created'}
                            = $created_ts;
                            $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}{$article_id}{'score'}
                            = $score;
                        }
                        delete $scorecard_report{$coach_name}{$coach}
{$author_name}{$quarter}{$article_id}{$unique_id};
                    }
...
{$author_name}{$quarter}{$article_id}{'score'};
                    $score_amount += $score;
...
{$quarter}{'sorter'};
                ...
                my $quarter_score = $scorecard_report{$coach_name}{$coach}
{$author_name}{$quarter}{'score'};
                $scorecard_report{$coach_name}{$coach}{$author_name}
{$quarter}
                = $quarter_score;
            }
...
{'unique articles scored'}
            ...
            # Yearly Average
            my $quarter_amount = scalar @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}};
            my $author_score_avg = $author_score_amount/$quarter_amount;
...
{$author_name}{'sorter'}} ) {
                ...
                if ( !grep { $_ eq $quarter } @{$scorecard_report{$coach_name}
{$coach}{'special_metrics'}} ) {

                    push @{$scorecard_report{$coach_name}{$coach}
{'special_metrics'}}, $quarter;
                }
            }
            delete $scorecard_report{$coach_name}{$coach}{$author_name}
{'sorter'};
...
foreach my $coach_name ( keys %scorecard_report ) {
    ...
    my $subject = "KCS Article Scorecard Yearly Team Summary for $coach_name"
                  ." for $year_name";
...
sub usage {
  ...
  DESCRIPTION:
    this script used to send out team yearly reports to scorecard in iKbase.
