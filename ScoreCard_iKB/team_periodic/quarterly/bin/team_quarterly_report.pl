...
use strict;
#======================================================================
# Description: manipulate and sent out the scorecard team quarterly report.
...
my %env_opts = (
                'development' =>
                    '../conf/team_quarterly_report_development.conf',
                'test'        =>
                    '../conf/team_quarterly_report_test.conf',
                'production'  =>
                    '../conf/team_quarterly_report_production.conf',
               );
...
foreach my $month ( @{$months->{'sorter'}} ) {
    ...
    while ( my $select_scores_rh = $select_scores_sth->fetchrow_hashref ) {
        ...
        $scorecard_report{$coach_name}{$coach}{$author_name}{$month}
{$article_id}{$unique_id}
        = \%score;

        push @{$scorecard_report{$coach_name}{$coach}{$author_name}{$month}
{$article_id}{'sorter'}},
        $unique_id;
...
{$coach}{'sorter'}}  ) {
        ...
        # sorting months
        if( !grep { $_ eq $month } @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}}) {
...
{$coach}{$author_name}{$month}{'sorter'}} ) {
            push @{$scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{'sorter'}}, $article_id;
        }
...
foreach my $coach_name ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$coach_name}} ) {
        ...
        foreach my $author_name
            ...
            foreach my $month ( @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}} ) {
...
{$month}{$article_id}{'new articles'}
                    = "0";
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'edited articles'}
                    = "0";

                    foreach my $unique_id ( @{$scorecard_report{$coach_name}
{$coach}{$author_name}{$month}{$article_id}{'sorter'}} ) {

                        my $created_ts = $scorecard_report{$coach_name}{$coach}
{$author_name}{$month}{$article_id}{$unique_id}{'created'};
...
{$author_name}{$month}{$article_id}{'new articles'} = "1";
                            }
                            elsif ($edited_flag > 0) {
                                $edited_article_scored ++;
                                $scorecard_report{$coach_name}{$coach}
{$author_name}{$month}{$article_id}{'edited articles'} = "1";
                            }
                        }
                        else {
                            $created_flag
...
{$author_name}{$month}{$article_id}{'new articles'} = "1";
                    ...
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'new article scored'}
                    = $new_article_scored;
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'edited article scored'}
                    = $edited_article_scored;

                    $total_times_scored
                    = $new_article_scored + $edited_article_scored;
                    $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'total times scored'}
                    = $total_times_scored;
...
{$author_name}{$month}{$article_id}{$unique_id}{'created'};
                            my $score
                            = $scorecard_report{$coach_name}{$coach}
{$author_name}{$month}{$article_id}{$unique_id}{'score'};

                            $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'created'}
                            = $created_ts;
                            $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}{$article_id}{'score'}
                            = $score;
                        }
                        delete $scorecard_report{$coach_name}{$coach}
{$author_name}{$month}{$article_id}{$unique_id};
                    }
...
{$author_name}{$month}{$article_id}{'score'};
                    $score_amount += $score;
...
{$month}{'sorter'};
                ...
                my $month_score = $scorecard_report{$coach_name}{$coach}
{$author_name}{$month}{'score'};
                $scorecard_report{$coach_name}{$coach}{$author_name}
{$month}
                = $month_score;
            }
...
{'unique articles scored'}
            ...
            # Quarterly Average
            my $month_amount = scalar @{$scorecard_report{$coach_name}{$coach}
{$author_name}{'sorter'}};
            my $author_score_avg = $author_score_amount/$month_amount;
...
{$author_name}{'sorter'}} ) {
                ...
                if ( !grep { $_ eq $month } @{$scorecard_report{$coach_name}
{$coach}{'special_metrics'}} ) {

                    push @{$scorecard_report{$coach_name}{$coach}
{'special_metrics'}}, $month;
                }
            }
            delete $scorecard_report{$coach_name}{$coach}{$author_name}
{'sorter'};
...
foreach my $coach_name ( keys %scorecard_report ) {
    ...
    my $subject = "KCS Article Scorecard Quarterly Team Summary for $coach_name"
                  ." for $quarter_name";
...
sub usage {
  ...
  DESCRIPTION:
    this script used to send out team quarterly reports to scorecard in iKbase.
