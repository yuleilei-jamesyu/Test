...
use strict;
#======================================================================
# Description: manipulate and sent out the scorecard individual monthly
...
my %env_opts = (
                'development' =>
                    '../conf/individual_monthly_report_development.conf',
                'test'        =>
                    '../conf/individual_monthly_report_test.conf',
                'production'  =>
                    '../conf/individual_monthly_report_production.conf',
               );
...
while ( my $select_scores_rh = $select_scores_sth->fetchrow_hashref ) {
    ...
    my $week_ending           = $select_scores_rh->{'week_ending'};
    ...
    if ($scorer_id eq $coach_id) {
        ...
        $scorer_email =~ s/@.+//g;
        $scorecard_report{$author_id}{$coach}{'category'} = "Scoring by Others";

        if (!grep {$_ eq $scorer_email} @{$scorecard_report{$author_id}{$coach}
{'scores'}{$week_ending}{$article_id}{'coaches'}}) {
            push @{$scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'coaches'}}, $scorer_email;
        }
    }
    $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}{$article_id}
{$unique_id} = \%score;
    push @{$scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
{$article_id}{'sorter'}}, $unique_id;

    # sorting weekends
    if( !grep { $_ eq $week_ending }
...
foreach my $author_id ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$author_id}} ) {
        ...
        foreach my $week_ending ( @{$scorecard_report{$author_id}{$coach}
{'scores'}{'sorter'}} ) {
                my %article_metrics = (
                    'new article scored' => 0,
                    'edited article scored' => 0,
...
{$week_ending}{$article_id}{'new articles'} = 0;
                $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'edited articles'} = 0;

                my $score_count = scalar @{$scorecard_report{$author_id}{$coach}
{'scores'}{$week_ending}{$article_id}{'sorter'}};
                my $score_num = 0;
...
{'scores'}{$week_ending}{$article_id}{$unique_id}{'created'};
                    ...
                    if ( defined $article_edited_ts ) {
                        ...
                        if ($created_flag < 0 && $edited_flag < 0) {
                            $new_article_scored ++;
                            $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'new articles'} = "1";
                        }
                        elsif ($edited_flag > 0) {
                            $edited_article_scored ++;
                            $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'edited articles'} = "1";
                        }
                    }
                    else {
                        $created_flag
...
{$week_ending}{$article_id}{'new articles'} = "1";
                    ...
                    if ($score_num == 1) {
                        ...
                        foreach my $metric ( @article_unusable ) {
                            ...
                            += $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{$unique_id}{$metric};
                        }
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{$unique_id}{'Article Unusable Total'}
                        = $article_unusable_total;
...
{$week_ending}{$article_id}{$unique_id}{$metric};
                        ...
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{$unique_id}{'Content Needs Rework Total'}
                        = $content_needs_rework_total;
...
{$week_ending}{$article_id}{$unique_id}{$metric};
                        ...
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{$unique_id}{'Field Needs Rework Total'}
                        = $field_needs_rework_total;

                        # calculating the amount for each metric_value for
                        # an article
                        foreach my $metric ( keys %{$scorecard_report
{$author_id}{$coach}{'scores'}{$week_ending}{$article_id}{$unique_id}} ) {
                            if ( exists $score_metrics{$metric} ) {
                                $score_metrics{$metric} += $scorecard_report
{$author_id}{$coach}{'scores'}{$week_ending}{$article_id}{$unique_id}{$metric};
                            }
                        }
                    }
                    delete $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{$unique_id};
                }
                delete $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'sorter'};

                $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
...
{$week_ending}{$article_id}{'new articles'} eq "1" ) {
                    $new_articles ++;
                }
                elsif ( $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id}{'edited articles'} eq "1" ) {
                    $edited_articles ++;
                }

                delete $scorecard_report{$author_id}{$coach}{'scores'}
{$week_ending}{$article_id};
            }

            my $article_count = scalar @{$scorecard_report{$author_id}{$coach}
{'scores'}{$week_ending}{'sorter'}};
            delete $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
{'sorter'};
...
{$week_ending}{$metric}
            ...
            # new/edited/total updated articles for per week
            $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
{'new articles'}
            = $new_articles;
            $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
...
{'total updated articles'}
            ...
            # unique articles scored for per week
            $scorecard_report{$author_id}{$coach}{'scores'}{$week_ending}
...
{'Starting Average Percentage'}
            ...
            foreach my $metric ( keys %week_end_metrics ) {
                $week_end_metrics{$metric} += $scorecard_report{$author_id}
{$coach}{'scores'}{$week_ending}{$metric};
            }
        }

        my $week_nums = scalar @{$scorecard_report{$author_id}{$coach}
{'scores'}{'sorter'}};
        foreach my $metric ( keys %week_end_metrics ) {
            my $total = $week_end_metrics{$metric};
            $scorecard_report{$author_id}{$coach}{'scores'}{'Total'}{$metric}
             = $total;

            my $average = $week_end_metrics{$metric}/$week_nums;
...
{'scores'}{$week_end}{$metric};

                if ( $metric_value == 0 ) {
                    $scorecard_report{$author_id}{$coach}{'scores'}{$week_end}
{$metric}
                    = "";
                }
                else {
...
foreach my $author_id ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$author_id}} ) {
        ...
        my $subject
        = "$report_tag - KCS Weekly Scorecard Summary for $author_name "
          . "for $month_name";
...
sub usage {
  ...
  DESCRIPTION:
    this script used to send out individual monthly reports to scorecard in iKbase.
