...
use strict;
#======================================================================
# Description: manipulate and sent out the scorecard individual yearly
...
my %env_opts = (
                'development' =>
                    '../conf/individual_yearly_report_development.conf',
                'test'        =>
                    '../conf/individual_yearly_report_test.conf',
                'production'  =>
                    '../conf/individual_yearly_report_production.conf',
               );
...
# datetime stuff
my $year_name = $config_vars_rh->{'year_name'};
die "year_name not defined"   unless ( defined $year_name );
...
my $quarters = {
               $quarter1_name => { 'start_datetime' => $quarter1_start,
                                   'stop_datetime' => $quarter1_end,
                                 },
               $quarter2_name => { 'start_datetime' => $quarter2_start,
                                   'stop_datetime' => $quarter2_end,
                                 },
               $quarter3_name => { 'start_datetime' => $quarter3_start,
                                   'stop_datetime' => $quarter3_end,
                                 },
               $quarter4_name => { 'start_datetime' => $quarter4_start,
                                   'stop_datetime' => $quarter4_end,
                                 },
               'sorter' => [$quarter1_name, $quarter2_name, $quarter3_name,
                            $quarter4_name],
             };
#print Dumper($quarters);
...
foreach my $quarter ( @{$quarters->{'sorter'}} ) {
    my $quarter_start = $quarters->{$quarter}{'start_datetime'};
    my $quarter_end   = $quarters->{$quarter}{'stop_datetime'};

    $select_scores_sth->execute($quarter_start, $quarter_end);
...
{$coach}{'scores'}{$quarter}{$article_id}{'coaches'}}) {
                push @{$scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'coaches'}}, $scorer_email;
            }
        }
        $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}{$article_id}
{$unique_id} = \%score;
        push @{$scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
{$article_id}{'sorter'}}, $unique_id;

        # sorting quarters
        if( !grep { $_ eq $quarter }
...
foreach my $author_id ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$author_id}} ) {
        ...
        foreach my $quarter ( @{$scorecard_report{$author_id}{$coach}
{'scores'}{'sorter'}} ) {
                my %article_metrics = (
                    'new article scored' => 0,
                    'edited article scored' => 0,
...
{$quarter}{$article_id}{'new articles'} = 0;
                $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'edited articles'} = 0;

                my $score_count = scalar @{$scorecard_report{$author_id}{$coach}
{'scores'}{$quarter}{$article_id}{'sorter'}};
                my $score_num = 0;
...
{'scores'}{$quarter}{$article_id}{$unique_id}{'created'};
                    ...
                    if ( defined $article_edited_ts ) {
                        ...
                        if ($created_flag < 0 && $edited_flag < 0) {
                            $new_article_scored ++;
                            $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'new articles'} = "1";
                        }
                        elsif ($edited_flag > 0) {
                            $edited_article_scored ++;
                            $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'edited articles'} = "1";
                        }
                    }
                    else {
                        $created_flag
...
{$quarter}{$article_id}{'new articles'} = "1";
                    ...
                    if ($score_num == 1) {
                        ...
                        foreach my $metric ( @article_unusable ) {
                            ...
                            += $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{$unique_id}{$metric};
                        }
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{$unique_id}{'Article Unusable Total'}
                        = $article_unusable_total;
...
{$quarter}{$article_id}{$unique_id}{$metric};
                        ...
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{$unique_id}{'Content Needs Rework Total'}
                        = $content_needs_rework_total;
...
{$quarter}{$article_id}{$unique_id}{$metric};
                        ...
                        $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{$unique_id}{'Field Needs Rework Total'}
                        = $field_needs_rework_total;

                        # calculating the amount for each metric_value for
                        # an article
                        foreach my $metric ( keys %{$scorecard_report
{$author_id}{$coach}{'scores'}{$quarter}{$article_id}{$unique_id}} ) {
                            if ( exists $score_metrics{$metric} ) {
                                $score_metrics{$metric} += $scorecard_report
{$author_id}{$coach}{'scores'}{$quarter}{$article_id}{$unique_id}{$metric};
                            }
                        }
                    }
                    delete $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{$unique_id};
                }
                delete $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'sorter'};

                $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
...
{$coach}{'scores'}{$quarter}{$article_id}} ) {
...
{'scores'}{$quarter}{'coaches'}} ) {

                                push @{$scorecard_report{$author_id}{$coach}
{'scores'}{$quarter}{'coaches'}}, $coach_name;
                            }
                        }
                    }
                    else {
...
{$quarter}{$article_id}{'new articles'} eq "1" ) {
                    $new_articles ++;
                }
                elsif ( $scorecard_report{$author_id}{$coach}{'scores'}
{$quarter}{$article_id}{'edited articles'} eq "1" ) {
                    $edited_articles ++;
                }
...
{'scores'}{$quarter}{'sorter'}};
            delete $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
{'sorter'};
...
{$quarter}{$metric}
            ...
            # new/edited/total updated articles for per quarter
            $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
{'new articles'}
            = $new_articles;
            $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
...
{'total updated articles'}
            ...
            # unique articles scored for per quarter
            $scorecard_report{$author_id}{$coach}{'scores'}{$quarter}
...
{'Starting Average Percentage'}
            ...
            foreach my $metric ( keys %quarter_metrics ) {
                $quarter_metrics{$metric} += $scorecard_report{$author_id}
{$coach}{'scores'}{$quarter}{$metric};
            }
        }

        my $quarter_nums = scalar @{$scorecard_report{$author_id}{$coach}
{'scores'}{'sorter'}};
        foreach my $metric ( keys %quarter_metrics ) {
            my $total = $quarter_metrics{$metric};
            $scorecard_report{$author_id}{$coach}{'scores'}{'Total'}{$metric}
             = $total;

            my $average = $quarter_metrics{$metric}/$quarter_nums;
...
foreach my $author_id ( keys %scorecard_report ) {
    ...
    foreach my $coach ( keys %{$scorecard_report{$author_id}} ) {
        ...
        my $subject
        = "$report_tag - KCS Quarterly Scorecard Summary for $author_name "
          . "for $year_name";
